"""
Lutron RadioRA2 or HomeWorks OS module for interacting with the Main Repeater.
...
class LutronConnection(threading.Thread):
  ...
  PROMPT_HW = b'QNET: '  # homeworks prompt
  PROMPT_RA = b'GNET> '  # radio ra prompt

  def __init__(self, host, user, password, recv_callback, is_radioRA=True):
  ...
  def _do_login_locked(self):
    ...
    self._telnet.read_until(
      LutronConnection.PROMPT_RA if self._is_radioRA else LutronConnection.PROMPT_HW,
      timeout=3)
    ...
    self._send_locked("#MONITORING,23,1")  # shade groups
  ...
  def run(self):
    ...
    """
    _LOGGER.info("Lutron Monitoring Started")
    try:
      self._main_loop()
    except Exception:
      _LOGGER.exception("Uncaught exception")
...
class LutronXmlDbParser(object):
  ...
  Lutron HW has both a SHADEGROUP and OUTPUT for each shade.
  We only use the SHADEGROUP, as the output can't handle the stop/up/down methods.
  """
  ...
  def _parse_area(self, area_xml):
    ...
    if not occupancy_group:
      _LOGGER.warning("Occupancy Group not found for Area: %s; ID: %s",
                      area_name, occupancy_group_id)
    ...
    for device_group in area_xml.find('DeviceGroups'):
      if device_group.tag == 'DeviceGroup':
        ...
        _LOGGER.info(f"Unknown tag in DeviceGroups child {device_group.tag}")
        devs = []
    ...
    # add shade groups as seperate devices
    for shade_group_xml in area_xml.find('ShadeGroups'):
      _LOGGER.debug(f'found shade group in area: {area_name}')
      shade_group = self._parse_shadegroup(shade_group_xml)
      area.add_shadegroup(shade_group)
    return area
  ...
  def _parse_shadegroup(self, shade_group_xml):
    """ parse a shade group which can go up and down"""
    kwargs = {
      'name': shade_group_xml.get('Name'),
      'integration_id': int(shade_group_xml.get('IntegrationID')),
      'uuid': shade_group_xml.get('UUID')
    }
    return Shadegroup(self._lutron, **kwargs)
...
class Lutron(object):
  ...
  def __init__(self, host, user, password, is_radioRA=True):
    ...
    self._name = None
    self._conn = LutronConnection(host, user, password, self._recv, is_radioRA)
    self._ids = {}
  ...
  def register_id(self, cmd_type, obj):
    ...
    notifications. This is the core mechanism how Output and Keypad and shadegroups objects get
  ...
  def _recv(self, line):
    ...
    args = parts[2:]
    # there are many 'unknown' commands that are emitted from the controller, just ignore
  ...
  def load_xml_db(self, cache_path=None, max_cache_age=7):
    """Load the Lutron database from the server.

    If a locally cached copy is available, use that instead. (or create it - with max age also given in days)
    """
      ...
      try:
        if max_cache_age > 0:
          ...
          age = (datetime.datetime.fromtimestamp(
            timestamp) - datetime.datetime.now()).days
        if age > 0 and age < max_cache_age:
...
class Output(LutronEntity):
  ...
  Although every ShadeGroup also has an output associated with it (which is
   not usually used to control the shade) """
  _CMD_TYPE = 'OUTPUT'
  ...
  def is_dimmable(self):
    ...
    return self.type not in (
      'NON_DIM', 'NON_DIM_INC', 'NON_DIM_ELV', 'EXHAUST_FAN_TYPE',
      'RELAY_LIGHTING') and not self.type.startswith(
      'CCO_')

  @property
  def is_motor(self):
    """True if this is a motor kind of output, like shade, motor etc - that can go up and down"""
    return self.type in ('MOTOR', 'SYSTEM_SHADE')

  @property
  def is_shade(self):
    """ True if this is a shade, should not be used directly! use SHADEGRP instead"""
    return self.type == 'SYSTEM_SHADE'

  def start_raise(self):
    """ handle motor raise function - applicable only to motor devices """
    if self.is_motor:
      self._lutron.send(Lutron.OP_EXECUTE, Output._CMD_TYPE,
      ...
      _LOGGER.warning(
        f"Raise action not allowed on this output {self._integration_id}")

  def start_lower(self):
    """ handle motor lower function - applicable only to motor devices """
    if self.is_motor:
      self._lutron.send(Lutron.OP_EXECUTE, Output._CMD_TYPE,
                        self._integration_id,
      ...
      _LOGGER.warning(
        f"Lower action not allowed on this output {self._integration_id}")

  def stop(self):
    """ handle motor stop function - applicable only to motor devices """
    if self.is_motor:
      self._lutron.send(Lutron.OP_EXECUTE, Output._CMD_TYPE,
                        self._integration_id,
                        Output._ACTION_STOP)
      ...
      _LOGGER.warning(
        f"Stop action not allowed on this output {self._integration_id}")


class Shadegroup(LutronEntity):
  """This is the Shadegroup entity in Lutron universe. This generally refers to a
  shade with up down go to level and stop"""
  _CMD_TYPE = 'SHADEGRP'
  # _CMD_TYPE_MOTOR = 'OUTPUT'  # motors use output version of the same command
  _ACTION_ZONE_LEVEL = 1
  _ACTION_ZONE_RAISE = 2
  _ACTION_ZONE_LOWER = 3
  _ACTION_ZONE_STOP = 4

  class Event(LutronEvent):
    """Shadegroup events that can be generated.

    LEVEL_CHANGED: The Shade level has changed.
        Params:
          level: new Shade level (float)
    """
    LEVEL_CHANGED = 1

  def __init__(self, lutron, name, integration_id, uuid):
    """Initializes the shadegroup/motor - set is_motor to True if this is the Motor and not shade """
    super(Shadegroup, self).__init__(lutron, name, uuid)

    self._level = 0.0
    self._query_waiters = _RequestHelper()
    self._integration_id = integration_id
    # self._is_motor=is_motor
    # self._CMD_TYPE=Shadegroup._CMD_TYPE_MOTOR if is_motor else Shadegroup._CMD_TYPE
    self._lutron.register_id(Shadegroup._CMD_TYPE, self)

  def __str__(self):
    """Returns a pretty-printed string for this object."""
    return 'Shadegroup name: "%s"  id: %d' % (
  ...
  def handle_update(self, args):
    """Handles an event update for this object, e.g. shade level change."""
    state = int(args[0])
    if state != Shadegroup._ACTION_ZONE_LEVEL:
      return False
    level = float(args[1])
    _LOGGER.debug("Updating Shade %d(%s): s=%d l=%f" % (
      self._integration_id, self._name, state, level))
    self._level = level
    self._query_waiters.notify()
    self._dispatch_event(Shadegroup.Event.LEVEL_CHANGED, {'level': self._level})
    return True

  def __do_query_level(self):
    """Helper to perform the actual query the current shade level of the
    Shade. For pure on/off loads the result is either 0.0 or 100.0."""
    self._lutron.send(Lutron.OP_QUERY, self._CMD_TYPE, self._integration_id,
                      Shadegroup._ACTION_ZONE_LEVEL)

  def last_level(self):
    """Returns last cached value of the shade level, no query is performed."""
    return self._level

  @property
  def level(self):
    """Returns the current shade level by querying the remote controller."""
    ev = self._query_waiters.request(self.__do_query_level)
    ev.wait(1.0)
    return self._level
  ...
  @level.setter
  def level(self, new_level):
    """Sets the new shade level."""
    _LOGGER.info(
      f"Setting shade new level: {self._integration_id} to level: {new_level}")
    if self._level == new_level:
      return
    self._lutron.send(Lutron.OP_EXECUTE, Shadegroup._CMD_TYPE,
                      self._integration_id,
                      Shadegroup._ACTION_ZONE_LEVEL, "%.2f" % new_level)
    self._level = new_level
  ...
  def start_raise(self):
    ...
    self._lutron.send(Lutron.OP_EXECUTE, Shadegroup._CMD_TYPE,
                      self._integration_id,
                      Shadegroup._ACTION_ZONE_RAISE)

  def start_lower(self):
    """Starts lowering the shade."""
    self._lutron.send(Lutron.OP_EXECUTE, Shadegroup._CMD_TYPE,
                      self._integration_id,
                      Shadegroup._ACTION_ZONE_LOWER)

  def stop(self):
    """Starts raising the shade."""
    self._lutron.send(Lutron.OP_EXECUTE, Shadegroup._CMD_TYPE,
                      self._integration_id,
                      Shadegroup._ACTION_ZONE_STOP)
...
class Keypad(LutronEntity):
  ...
  def __init__(self, lutron, name, keypad_type, location, integration_id, uuid):
    ...
    _LOGGER.debug(
      f"KEYPAD - registring id= {self._integration_id}- {Output._CMD_TYPE}")
  ...
  def handle_update(self, args):
    ...
    _LOGGER.debug("Updating Keypad %d(%s): c=%d a=%d params=%s" % (
...
class MotionSensor(LutronEntity):
  ...
  def __init__(self, lutron, name, integration_id, uuid):
    ...
    self._power = None
    _LOGGER.debug(
      f"MOTHION - registring id= {self._integration_id}- {Output._CMD_TYPE}")
  ...
  def _do_query_battery(self):
    ...
    return self._lutron.send(Lutron.OP_QUERY, MotionSensor._CMD_TYPE,
                             self._integration_id,
...
class OccupancyGroup(LutronEntity):
  ...
  def _bind_area(self, area):
    ...
    _LOGGER.debug(
      f"OccupancyGroup - registring id= {self._integration_id}- {Output._CMD_TYPE}")
  ...
  def _do_query_state(self):
    ...
    return self._lutron.send(Lutron.OP_QUERY, OccupancyGroup._CMD_TYPE,
                             self._integration_id,
...
class Area(object):
  ...
  def add_shadegroup(self, shadegroup):
    """Adds an output object that's part of this area, only used during
    initial parsing."""
    self._shadegroups.append(shadegroup)
  ...
  @property
  def shadegroups(self):
    """Return the tuple of the shade groups from this area."""
    return tuple(shadegroup for shadegroup in self._shadegroups)
