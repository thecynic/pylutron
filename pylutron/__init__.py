...
class LutronConnection(threading.Thread):
    ...
    def _do_login_locked(self):
        ...
        self._telnet.read_until(LutronConnection.PROMPT_RA if self._is_radioRA else LutronConnection.PROMPT_HW,
                                timeout=3)
...
class LutronXmlDbParser(object):
    ...
    Lutron HW has both a SHADEGROUP and OUTPUT for each shade.  We only use the SHADEGROUP, as the output can't handle the stop/up/down methods.
    """
        ...
        if cache_path:
            try:
                if max_cache_age > 0:
                    ...
                    age = (datetime.datetime.fromtimestamp(timestamp) - datetime.datetime.now()).days
...
class Output(LutronEntity):
    ...
    switched/dimmed load, e.g. light fixture, outlet, etc. Although every ShadeGroup also has an output associated with i too"""
    _CMD_TYPE = 'OUTPUT'
    ...
    def is_dimmable(self):
        ...
        return self.type not in (
            'NON_DIM', 'NON_DIM_INC', 'NON_DIM_ELV', 'EXHAUST_FAN_TYPE', 'RELAY_LIGHTING') and not self.type.startswith(
            'CCO_')
    ...
    def start_raise(self):
        ...
        if self.is_motor:
            ...
            _LOGGER.warning(f"Raise action not allowed on this output {self._integration_id}")
    ...
    def start_lower(self):
        ...
        if self.is_motor:
            ...
            _LOGGER.warning(f"Lower action not allowed on this output {self._integration_id}")
    ...
    def stop(self):
        ...
        if self.is_motor:
            ...
            _LOGGER.warning(f"Stop action not allowed on this output {self._integration_id}")
...
class Shadegroup(LutronEntity):
    ...
    def level(self, new_level):
        ...
        _LOGGER.info(f"Setting shade new level: {self._integration_id} to level: {new_level}")
        if self._level == new_level:
            return
        self._lutron.send(Lutron.OP_EXECUTE, Shadegroup._CMD_TYPE, self._integration_id,
...
class Keypad(LutronEntity):
    ...
    def __init__(self, lutron, name, keypad_type, location, integration_id, uuid):
        ...
        _LOGGER.debug(f"KEYPAD - registring id= {self._integration_id}- {Output._CMD_TYPE}")
...
class MotionSensor(LutronEntity):
    ...
    def __init__(self, lutron, name, integration_id, uuid):
        ...
        self._battery = None
        self._power = None
        _LOGGER.debug(f"MOTHION - registring id= {self._integration_id}- {Output._CMD_TYPE}")
...
class OccupancyGroup(LutronEntity):
    ...
    def _bind_area(self, area):
        ...
        _LOGGER.debug(f"OccupancyGroup - registring id= {self._integration_id}- {Output._CMD_TYPE}")
